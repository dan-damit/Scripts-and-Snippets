...
param(
    ...
    [string]$WorkingDir = 'C:\Program Files (x86)\Microsoft Dynamics\GP2018\Timeclk',
    [string]$ExePath = 'C:\Program Files (x86)\Microsoft Dynamics\GP2018\Timeclk\Dynamics.exe',
...
# Define Native methods for session ID retrieval
if (-not ([System.Management.Automation.PSTypeName]'SessionApi').Type) {
...
$logPath = Join-Path $logDir ("Run-GPTimeclk_" + (Get-Date -Format 'yyyyMMdd_HHmmss') + ".log")
...
function Get-ActiveConsoleSessionId {
    try {
        $sid = [Native.Session]::WTSGetActiveConsoleSessionId()
        if ($sid -eq 0xFFFFFFFF) { return $null } # INVALID_SESSION_ID
        return [int]$sid
    }
    catch {
        return $null
...
function Get-LaunchingSessionId {
    # Session of the current PowerShell host process
    try {
...
try {
    ...
    $launchingSid = Get-LaunchingSessionId
    $activeConsoleSid = Get-ActiveConsoleSessionId

    if (-not $sessionId) {
        if ($null -ne $activeConsoleSid) {
            $sessionId = $activeConsoleSid
            Write-Log "Active session not found via qwinsta; defaulting to active console session ID $sessionId."
        }
        elseif ($null -ne $launchingSid) {
            $sessionId = $launchingSid
            Write-Log "Active session not found via qwinsta; falling back to launching session ID $sessionId."
        }
        else {
            $sessionId = 1
            Write-Log "Active session not found via qwinsta and API fallback failed; defaulting to 1."
        }
    }
    else {
        Write-Log "Active session detected via qwinsta: ID=$sessionId"
    }
