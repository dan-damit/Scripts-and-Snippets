...
function Get-BatteryHealth {
    ...
    # Resolve defaults from config when parameters not supplied
    $cfg = Get-TechToolboxConfig
    $br = $cfg.BatteryReport
    if (-not $PSBoundParameters.ContainsKey('ReportPath') -or [string]::IsNullOrWhiteSpace($ReportPath)) { $ReportPath = $br.ReportPath }
    if (-not $PSBoundParameters.ContainsKey('OutputJson') -or [string]::IsNullOrWhiteSpace($OutputJson)) { $OutputJson = $br.OutputJson }
    if (-not $PSBoundParameters.ContainsKey('DebugInfo') -or [string]::IsNullOrWhiteSpace($DebugInfo)) { $DebugInfo = $br.DebugInfo }
