...
function Export-MessageTraceResults {
    ...
    try {
        ...
        $sumPath = Join-Path $ExportFolder ("MessageTraceSummary_{0}.csv" -f $ts)
        $detPath = Join-Path $ExportFolder ("MessageTraceDetails_{0}.csv" -f $ts)
        ...
        if ($Details -and $Details.Count -gt 0) {
            if ($PSCmdlet.ShouldProcess($detPath, 'Export details CSV')) {
                $Details | Export-Csv -Path $detPath -NoTypeInformation -Encoding UTF8
            }
        }
