function Clear-BrowserProfileData {
    ...
    begin {
        ...
        Write-Log -Level Info -Message ("Starting browser profile cleanup. " +
            "Browser={0}, IncludeCache={1}, IncludeCookies={2}, SkipLocalStorage={3}, KillProcesses={4}, SleepAfterKillMs={5}" -f `
                $Browser, $IncludeCache, $IncludeCookies, $SkipLocalStorage, $KillProcesses, $SleepAfterKillMs)

        # Apply config-driven defaults only if the parameter wasn't provided and the config has a non-null value.
        if (-not $PSBoundParameters.ContainsKey('IncludeCache') -and $null -ne $bc.includeCache) { $IncludeCache = [bool]$bc.includeCache }
        if (-not $PSBoundParameters.ContainsKey('IncludeCookies') -and $null -ne $bc.includeCookies) { $IncludeCookies = [bool]$bc.includeCookies }
        if (-not $PSBoundParameters.ContainsKey('SkipLocalStorage') -and $null -ne $bc.skipLocalStorage) { $SkipLocalStorage = [bool]$bc.skipLocalStorage }
        if (-not $PSBoundParameters.ContainsKey('Browser') -and [string]::IsNullOrWhiteSpace($Browser) -and $null -ne $bc.defaultBrowser) {
        ...
        if (-not $PSBoundParameters.ContainsKey('Profiles') -and $null -ne $bc.defaultProfiles) {
            if ($bc.defaultProfiles -is [System.Collections.IEnumerable] -and -not ($bc.defaultProfiles -is [string])) {
                $Profiles = @($bc.defaultProfiles)
            }
            else {
                $Profiles = @("$($bc.defaultProfiles)")
            }
        }
        if (-not $PSBoundParameters.ContainsKey('KillProcesses') -and $null -ne $bc.killProcesses) { $KillProcesses = [bool]$bc.killProcesses }

        if (-not $PSBoundParameters.ContainsKey('SleepAfterKillMs') -and $null -ne $bc.sleepAfterKillMs) {
            $SleepAfterKillMs = [int]$bc.sleepAfterKillMs
        }
    }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBx0cNGTdO6xd+h
# PWabZa5KUTZG5Gk9duvsMPZ4cuB6FqCCGEowggUMMIIC9KADAgECAhAR+U4xG7FH
...
# AQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCB/pbQ3Kyp5
# bohqvJ7fzCEzetKIaSpvEvLptd4803Jv+TANBgkqhkiG9w0BAQEFAASCAgCttUeU
# CIt4/A3Nv3/oDirO6Bw/hnz8nSKuMsi0daW9D8VYtfmocikvRXan9duXmE0ml8ff
# z+r66y/d9MneeSwDEXwPSOSIHljqgL33Z4Dj5zKZgY1oNUvKTHBw0M6dgoM98P1c
# QBPUzFFgpk/szClodFnUfqRz25MP4sIpTxPDiTEOrj/PE098Xq4IZQQBNnnpBpeC
# ETwgzDRBuUAYOWx5wIRNSRyl3qq/GC7KXfzhg9gUdKonLO6tgPh9AVh1Ac/zp6oc
# TcoViEGDzn5aJnu5lPqeJPc1mxTeLBN9pQpSOxqi5gFYxvxv5O0nsAaB+MldQvhK
